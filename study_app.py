...
def main_app():
    ...
    if 'agendas' not in user_data: user_data['agendas'] = {} # Garante agenda
    ...
    if user == ADMIN_USER:
        tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["📊 Diário & Árvore", "📈 Análise e Dashboard", "🏆 Ranking Global", "📢 Alertas do Mentor", "📅 Agenda de Guerra", "🤖 Oráculo IA", "🛡️ Moderação"])
        current_tabs = [tab1, tab2, tab3, tab4, tab5, tab6, tab7]
    else:
        tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📊 Diário & Árvore", "📈 Análise e Dashboard", "🏆 Ranking Global", "📢 Alertas do Mentor", "📅 Agenda de Guerra", "🤖 Oráculo IA"])
        current_tabs = [tab1, tab2, tab3, tab4, tab5, tab6]
        ...
        if len(user_data['logs']) > 0:
            ...
            if not df_filtered.empty:
                ...
                if subject_mins:
                    ...
                    percentages = [(x/total_size)*100 for x in sizes]
                    legend_labels = [f"{p:.1f}% - {l}" for p, l in zip(percentages, labels)]
                    ...
                    plt.setp(ax.get_legend().get_title(), color='#333333') 
    ...
    # --- ABA 7: MODERAÇÃO (NOVA E EXCLUSIVA) ---
    if user == ADMIN_USER:
        with current_tabs[6]:
