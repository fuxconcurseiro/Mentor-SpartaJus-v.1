...
# --- FUSO HORÁRIO BRASÍLIA (CORREÇÃO) ---
# Cria um objeto de fuso horário UTC-3
...
def calculate_streak(logs):
    ...
    streak = 0
    # USA DATA DE BRASILIA PARA CÁLCULO CORRETO
    today = get_today_br()
...
def main_app():
    ...
    with c1: st.markdown(f"<div class='rank-card'><h2>{user.upper()}</h2><h3>🛡️ {get_patent(total_q)}</h3><p>Questões: {total_q} | 🔥 Fogo: {streak} dias</p></div>", unsafe_allow_html=True)
    with c2:
        ...
        with c_form:
            ...
            with st.form("log_form"):
                ...
                if st.form_submit_button("💾 Salvar"):
                    ...
                    new_log = {"data": d_log.strftime("%Y-%m-%d"), "acordou": wt, "dormiu": sl, "paginas": pg, "series": ws, "questoes": total_q_day, "questoes_detalhadas": q_details, "estudou": is_study}
                    exists = False
    ...
    with tabs[1]:
        ...
        if user_data['logs']:
            ...
            edited = st.data_editor(df_hist[['data', 'paginas', 'series', 'questoes', 'detalhes_str']], column_config={"detalhes_str": st.column_config.TextColumn("Detalhes (Mat: Qtd)", help="Ex: Const: 10"), "questoes": st.column_config.NumberColumn("Total Q", disabled=True)}, use_container_width=True, num_rows="dynamic")
            if st.button("Salvar Correções"):
                ...
                for _, r in edited.iterrows():
                    ...
                    nl.append({"data": r['data'], "acordou": orig_log.get('acordou', '00:00'), "dormiu": orig_log.get('dormiu', '00:00'), "paginas": int(r['paginas']), "series": int(r['series']), "questoes": tq, "questoes_detalhadas": new_dets, "estudou": (int(r['paginas']) > 0 or tq > 0)})
                user_data['logs'] = nl
                save_current_user_data()
